...
usage() {
  cat <<'EOF'
Uso:
  ./scripts/server/test-production.sh [opciones]

Opciones:
  --force             Fuerza envío del reporte borrando el stamp diario antes de ejecutar.
  --no-telegram-test  No envía mensaje TEST a Telegram; solo prueba el servicio.
  --direct            Ejecuta /opt/boot-report/boot-report.sh directo, sin systemd.
  --since-min N       Ventana de logs para journalctl. Default: 15.
  -h, --help          Ayuda.

Notas:
  - Ejecutar como usuario root en el servidor instalado.
...
echo "== Boot Report: test manual productivo =="
...
if curl -fsS "https://api.telegram.org/bot${BOT_TOKEN}/getMe" | grep -q '"ok"\s*:\s*true'; then
  echo "OK: el BOT_TOKEN responde."
else
  echo "ERROR: Telegram no respondió OK. Revisar BOT_TOKEN, DNS o salida a internet." >&2
  exit 1
fi
...
if [[ "${SEND_TEST_MSG}" -eq 1 ]]; then
  ...
  TEST_TEXT="<b>BOOT REPORT TEST</b>%0AHost: <code>$(hostname)</code>%0AFecha: <code>$(date '+%Y-%m-%d %H:%M:%S %Z')</code>%0AOrigen: <code>scripts/server/test-production.sh</code>"
  if curl -fsS -X POST "https://api.telegram.org/bot${BOT_TOKEN}/sendMessage" \
    ...
    echo "OK: mensaje TEST enviado."
  else
    echo "ERROR: falló sendMessage. Revisar CHAT_ID y permisos del bot." >&2
    exit 1
  fi
fi
...
echo ""
echo "OK: test productivo completado."
